...
class PatientStoriesCarousel extends React.Component {
  static propTypes = {
  ...
  render () {
    ...
    const imgChildren = dataArray.map((item, i) => (
      <Element key={i} style={{ background: item.color }}>
        {/* <QueueAnim
          ...
          key="img-wrapper"
        > */}
          <div className={`details-switch-demo-map map${i}`} key="map">
            <img src={item.map} width="100%" alt="dsdsds" />
          </div>
          <div className={`details-switch-demo-pic pic${i}`} key="pic">
    ...
    console.log(`IMage  ::  text  ${imgChildren}  ${textChildren}`)
    return (<div
